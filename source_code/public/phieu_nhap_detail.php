...
// Lấy phiếu
$stmt = $conn->prepare("SELECT p.*, u.username AS user_name FROM phieu_nhap p LEFT JOIN users u ON p.created_by = u.id WHERE p.id = ? LIMIT 1");
...
// Lấy chi tiết
$stmt2 = $conn->prepare("SELECT ct.id_san_pham, sp.ten_san_pham, ct.so_luong, ct.gia FROM chi_tiet_nhap ct JOIN san_pham sp ON sp.id = ct.id_san_pham WHERE ct.id_phieu_nhap = ?");
...
<div class="container mt-4">
  <h3>Chi tiết Phiếu Nhập #<?= $ph['id'] ?></h3>
  ...
  <a href="phieu_nhap_list.php" class="btn btn-outline-secondary">Quay lại</a>
</div>
